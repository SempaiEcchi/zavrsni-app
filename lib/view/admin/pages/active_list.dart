...
class CollapsingListActive extends ConsumerStatefulWidget {
  final Map<JobOpportunity, List<EmployedApplicant>> applications;
  final VoidCallback onRefresh;

  CollapsingListActive(this.applications, this.onRefresh);

  @override
  ConsumerState<CollapsingListActive> createState() => _CollapsingListState();
}

class _CollapsingListState extends ConsumerState<CollapsingListActive> {
  @override
    ...
    return ListView(
      ...
      children: [
        for (var entry in widget.applications.entries) ...[
          ...
          for (EmployedApplicant applicant in entry.value)
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 8.0),
              child: Row(
                children: [
                  Expanded(
                    child: EmployedApplicantListTile(
                      applicant: applicant,
                  ...
                  Column(
                    children: [
                      ...
                      // SmallPrimaryButton(onTap: () {}, text: "Ocijeni"),
                    ],
                  ),
                ],
              ),
...
class EmployedApplicantListTile extends StatelessWidget {
  final JobOpportunity jobOpportunity;
  final EmployedApplicant applicant;
  final int unreadMessageCount;

  const EmployedApplicantListTile({
    Key? key,
